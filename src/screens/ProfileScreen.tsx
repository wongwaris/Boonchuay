...
export default function ProfileScreen() {
  ...
  const { workouts, streak } = useHistory();
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsRow}>
          <StatBox label="Workouts" value={workouts.length.toString()} color={colors.accent} />
          <StatBox label="Streak" value={`${streak}d`} color={colors.amber} />
          <StatBox label="Minutes" value={totalMinutes > 999 ? `${Math.round(totalMinutes / 60)}h` : `${totalMinutes}`} color={colors.teal} />
          <StatBox label="Volume" value={totalVolume > 999 ? `${Math.round(totalVolume / 1000)}k` : `${totalVolume}`} color={colors.purple} />
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.divider} />
          <InfoRow
            icon={weightDiff < 0 ? 'trending-down' : weightDiff > 0 ? 'trending-up' : 'check'}
            label="To goal"
